...
def quadratic(x, a, b, c):
    ...
    lower = lower_bound(x)
    mid = -b / (2 * a) 
    f_upper, f_lower, f_mid = f(upper), f(lower), f(mid)
    if lower <= mid <= upper:
        return interval(min(f_upper, f_mid, f_lower), max(f_upper, f_mid, f_lower))
    else:
        return interval(min(f_upper, f_lower), max(f_lower, f_upper))
